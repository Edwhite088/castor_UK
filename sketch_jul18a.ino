...
float smoothed_arm1[10][10] = {0};
float smoothed_arm2[10][10] = {0};

const float alpha = 0.2;  // Smoothing factor (0.1–0.3 is typical)
bool firstPass = true;
...
void loop() {
  for (int i = 0; i < power_line; i++) {
    ...
    for (int ch = 0; ch < 6; ch++) {
      ...
      arm1[i][ch] = value;
      if (firstPass) smoothed_arm1[i][ch] = value;
      else smoothed_arm1[i][ch] = alpha * value + (1 - alpha) * smoothed_arm1[i][ch];
    }
    ...
    for (int ch = 0; ch < 6; ch++) {
      ...
      arm2[i][ch] = value;
      if (firstPass) smoothed_arm2[i][ch] = value;
      else smoothed_arm2[i][ch] = alpha * value + (1 - alpha) * smoothed_arm2[i][ch];
    }

    // Read extra analog pins for arm1
    for (int j = 0; j < numreadpin; j++) {
      int raw = analogRead(pinArm1[j]);
      float voltage = raw * (3.3 / 4095.0);
      arm1[i][j + 6] = voltage;
      if (firstPass) smoothed_arm1[i][j + 6] = voltage;
      else smoothed_arm1[i][j + 6] = alpha * voltage + (1 - alpha) * smoothed_arm1[i][j + 6];
    }

    // Read extra analog pins for arm2
    for (int j = 0; j < numreadpin; j++) {
      int raw = analogRead(pinArm2[j]);
      float voltage = raw * (3.3 / 4095.0);
      arm2[i][j + 6] = voltage;
      if (firstPass) smoothed_arm2[i][j + 6] = voltage;
      else smoothed_arm2[i][j + 6] = alpha * voltage + (1 - alpha) * smoothed_arm2[i][j + 6];
    }
...
void printArrays() {
  ...
  for (int i = 0; i < power_line; i++) {
    for (int j = 0; j < 10; j++) {
      Serial.print(smoothed_arm1[i][j], 3);
      Serial.print(",");
    }
  }
  ...
  for (int i = 0; i < power_line; i++) {
    for (int j = 0; j < 10; j++) {
      Serial.print(smoothed_arm2[i][j], 3);
